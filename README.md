...
The repository includes example applications for both Android and iOS that demonstrate how to use the library in each platform. The Android example uses Jetpack Compose, while the iOS example uses UIKit. Both examples display the Fibonacci sequence generated by the library, showing how platform-specific implementations can be used from a common codebase.
